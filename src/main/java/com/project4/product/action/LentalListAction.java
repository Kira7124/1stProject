...
public class LentalListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int count = dao.getProductCount("대여");
        ...
        if (category != null && !category.isEmpty()) {
            productList = dao.getProductList("대여", category);
            System.out.println(category + " 있음");
        }
        else if (company != null && !company.isEmpty()) {
        	productList = dao.getProductList1("대여", company);
        	System.out.println(company + " 있음");
        }
        else {
            productList = dao.getProductList("대여");
		...
		forward.setPath("./product/lentalList.jsp");
