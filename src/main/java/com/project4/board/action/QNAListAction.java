...
public class QNAListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(search == null) {
			 count = bdao.getQNABoardCount();
		}else {
			count = bdao.getQNABoardCount(search);
		}
		...
		if (count > 0 && search ==null) {
			Qna = bdao.getQnaBoardList(startRow, pageSize);
		}else if(count>0 && search !=null) {
			Qna = bdao.getQnaBoardList(startRow, pageSize, search);
		}else {
