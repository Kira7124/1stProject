...
<body>
	...
	<div id="product-row">
		<div class="productWrite-bg mb-4">
			<div class="card-header pb-0">
				...
				<tr class="writeForm">
					...
					<td>
						<c:if test="${not empty dto.board_file}">
				            <a href="./upload/${dto.board_file}" download="${dto.board_file}">
				                ${dto.board_file}
				            </a>
       					 </c:if>
				...
				<div>
					<c:if test="${sessionScope.mdto.id eq dto.id || sessionScope.mdto.id eq 'admin' }">
						<input type="button" value="수정하기" id="button" onclick="location.href='./QNAUpdate.bo?board_bno=${dto.board_bno}&pageNum=${pageNum }&id=${sessionScope.id }&board_subject=${dto.board_subject }&board_content=${dto.board_content }';">
					...
					<input type="button" value="답글쓰기" id="button"
			...
			<div>
				<input type="button" value="목록으로" id="button" onclick="location.href='QNA.bo?pageNum=${pageNum}'">
			</div>
			</div>
			</div>
		...
		<script type="text/javascript">
			function confirmDelete(boardBno, pageNum) {
		        ...
		        if (confirmation) {
		            location.href = 'QNADeleteAction.bo?board_bno=' + boardBno + '&pageNum=' + encodeURIComponent(pageNum);
		        } else {
