	            ...
	            <table class="saledetail-table">
	            	<tbody>
	            		...
	            		<tr class="saledetail-tr">
	            			...
	            			<td class="td_sit_sel light">
	            				<input type="text" id="rentalStartDate" placeholder="&nbsp;&nbsp;대여 시작일" readonly>&nbsp;&nbsp;부터&nbsp;&nbsp;
        						<input type="text" id="rentalEndDate" placeholder="&nbsp;&nbsp;대여 종료일" readonly>&nbsp;&nbsp;까지
	            ...
	            <div class="item-total-buttons">
		            <C:choose>
						<C:when test="${not empty sessionScope.mdto }">
			            	...
			            	<button type="submit" id="PayButton" class="item-de" value="대여">바로 대여하기</button>
		            	</C:when>
		            	<C:otherwise>
			            	<button type="submit" class="item-vi" value="장바구니" onclick="loginForm()">장바구니 담기</button>
			            	<button type="submit" class="item-de" value="대여" onclick="loginForm()">바로 대여하기</button>
		            	</C:otherwise>
	            	</C:choose>
	            </div>
			</div>
		...
		document.addEventListener("DOMContentLoaded", function() {
		    ...
		    radioGroups.forEach(function(radio) {
		        radio.addEventListener("change", function() {
		            ...
		            $("#rentalStartDate").val("");  // 대여 시작일을 빈 문자열로 설정
		            $("#rentalEndDate").val("");    // 대여 종료일을 빈 문자열로 설정
		            $("#rentalDuration").text("");	// 총 대여기간 빈 문자열로 설정
		            
		            // 시작일과 종료일을 위한 Datepicker 초기화
				    $("#rentalStartDate").datepicker({
				        dateFormat: 'yy-mm-dd',
				        minDate: 0,
				        onSelect: function(selectedDate) {
				            var endDatePicker = $("#rentalEndDate");
				    ...
				    $("#rentalEndDate").datepicker({
				        ...
				        minDate: 0,
				        onSelect: function(selectedDate) {
				            var startDatePicker = $("#rentalStartDate");
				    ...
				    function calculateAndDisplayDuration() {
				        var startDate = $("#rentalStartDate").datepicker("getDate");
				        var endDate = $("#rentalEndDate").datepicker("getDate");
				        var currentPrice1 = parseInt(itTotPriceElement.innerText.replace(/[^0-9]/g, ''));
				        ...
				        if (startDate && endDate) {
				            ...
				            // 대여 기간을 HTML 요소에 표시
				            $("#rentalDuration").text(" (" + daysDiff + "일)");
				            console.log(currentPrice);
				            console.log(daysDiff);
				            
				            currentPrice1 = currentPrice * daysDiff;
				            
				            // 예상 매입 가격을 화면에 업데이트합니다.
				            itTotPriceElement.innerText = currentPrice1.toLocaleString() + "원";
				            // 숫자 애니메이션 클래스를 추가하여 애니메이션 효과 부여
				            itTotPriceElement.classList.add('number-animation');
				            setTimeout(function() {
				                itTotPriceElement.classList.remove('number-animation');
			    ...
			    CartButton.addEventListener('click', function () {
			        ...
			        var rentalDuration = $("#rentalDuration").text();
			        
			        if (!rentalStartDate || !rentalEndDate) {
			            // If rental start date or end date is not selected, show an alert and return
			            alert("대여 시작일과 종료일을 선택하세요.");
			            return;
			        }
			        ...
			        var newUrl =
			            ...
			            "&rentalStartDate=" + 
			            encodeURIComponent(rentalStartDate) +
			            "&rentalEndDate=" +
			            encodeURIComponent(rentalEndDate)+
			            "&rentalDuration=" +
			            encodeURIComponent(rentalDuration);
