...
<body>
	...
	<div class="row">
		...
		<div class="purchase-container">
			...
			<!-- 		<button id="toggleButton">
					...
					<div class="purchaseOrder-detail">
						이메일 : <input id="input3" type="text" name="email" placeholder="이메일을 작성하세요">
					</div>
					<div class="purchaseOrder-detail">
						휴대폰 번호 : <input id="input2" type="text" name="phone_num" placeholder="휴대폰 번호를 작성하세요 ex)010-1234-5678">
					</div>
	
					<div class="purchaseOrder-detail">
						주소 :
						<div style="float: right;">
							<input type="text" id="sample6_postcode" name="zip_code" placeholder="우편번호"> <input type="button" id="sample6_button" onclick="sample6_execDaumPostcode()" value="주소 검색" style="outline: none;"><br> <input type="text" id="sample6_address" name="address" placeholder="주소"><br> <input type="text" id="sample6_detailAddress" name="address_detail" placeholder="상세주소"> <input type="text" id="sample6_extraAddress" name="refrence" placeholder="참고항목">
						</div>
					</div>
			</div>
			...
			<!-- 		<button id="toggleButton2">
				...
				<div class="lental-notice1">
					...
					<p>
						대여기간:
						<%=rentalStartDate%> ~ <%=rentalEndDate %><%=rentalDuration %></p>
					<p>
						총 대여가격:
						<%=formattedPrice%>원
					</p>
				...
				<div class="order-pay">
					...
					<p>-------------------------------</p>
					<button id="payment-request-button" onclick="handlePayment()" data-id="<%= session.getAttribute("id") %>">대여 결제</button>
	...
	<script>
		function sample6_execDaumPostcode() {new daum.Postcode({oncomplete : function(data) {
	...
	<script>
		document.addEventListener('DOMContentLoaded',function() {
			var formContent = document
					.getElementById('formContent');
			var toggleButton = document
					.getElementById('toggleButton');
			var formContent2 = document
					.getElementById('formContent2'); // New form ID
			var toggleButton2 = document
					.getElementById('toggleButton2'); // New button ID

			formContent.classList.remove('hidden'); // 폼 내용 보이도록 클래스 제거
			toggleButton.innerHTML = '<i class="fas fa-chevron-up"></i>'; // 화살표 위 아이콘

			formContent2.classList.remove('hidden'); // 폼 내용 보이도록 클래스 제거
			toggleButton2.innerHTML = '<i class="fas fa-chevron-up"></i>'; // 화살표 위 아이콘

			toggleButton.addEventListener('click',function() {
				formContent.classList
						.toggle('hidden'); // hidden 클래스를 토글
				if (formContent.classList
						.contains('hidden')) {
					toggleButton.innerHTML = '<i class="fas fa-chevron-down"></i> '; // 화살표 아래 아이콘
				} else {
					toggleButton.innerHTML = '<i class="fas fa-chevron-up"></i> '; // 화살표 위 아이콘
				}
			});
			toggleButton2.addEventListener('click',function() {
				formContent2.classList
						.toggle('hidden');
				if (formContent2.classList
						.contains('hidden')) {
					toggleButton2.innerHTML = '<i class="fas fa-chevron-down"></i>';
				} else {
					toggleButton2.innerHTML = '<i class="fas fa-chevron-up"></i>';
				}
			});
		});
	</script>
	...
	function initiatePayment() {
		var id = '<%= session.getAttribute("id") %>';
		 var name = document.getElementById("input1").value;
		    var phoneNum = document.getElementById("input1").value;
		    var zipCode = document.getElementById("sample6_postcode").value;
		    var address = document.getElementById("sample6_address").value;
		    var addressDetail = document.getElementById("sample6_detailAddress").value;
		    var email = document.getElementById("input3").value;
	    ...
	    paymentRequestButton.addEventListener("click", () => {
	        paymentWidget.requestPayment({
	            ...
	            orderName: "<%=name%>",
	            successUrl: window.location.origin + "/Project4/rentalSuccessAction.od?price=<%=price%>&name=<%=name%>&status1=<%=status1%>&status2=<%=status2%>&rentalStartDate=<%=rentalStartDate%>&rentalEndDate=<%=rentalEndDate%>&img=<%=imageURL%>&id=" + id + "&name=" + name + "&phone_num=" + phoneNum + "&zip_code=" + zipCode + "&address=" + address + "&address_detail=" + addressDetail+"&email="+email,
